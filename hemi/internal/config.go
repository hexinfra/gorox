...
func parseComponent1[M Component, T Component, C any](c *config, sign token, mesher M, create func(sign string, name string) T, kase *C, assign func(T)) { // dealer, filter
	name := sign.text
	if current := c.forward(); current.kind == tokenString {
		name = current.text
		c.forward()
