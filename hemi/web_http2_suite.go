...
type server2Conn struct {
	...
	incomingChan   chan any            // frames and errors generated by c.receiver() and waiting for c.serve() to consume
...
func (c *server2Conn) MakeTempName(to []byte, unixTime int64) int {
...
func (c *server2Conn) serve() { // runner
	...
	go c.receiver()
serve:
	for { // each frame from c.receiver() and streams
		select {
		case incoming := <-c.incomingChan: // got an incoming frame from c.receiver()
			if inFrame, ok := incoming.(*http2InFrame); ok { // data, headers, priority, rst_stream, settings, ping, windows_update, unknown
				...
				// c.serve() was broken, but c.receiver() was not. need wait
				c.waitReceive = true
			...
			break serve
		case outFrame := <-c.outgoingChan: // got an outgoing frame from streams. only headers and data!
...
func (c *server2Conn) _handshake() error {
	...
	if err := c._updateClientSettings(prefaceInFrame); err != nil {
		return err
	}
	// TODO: write deadline
...
func (c *server2Conn) receiver() { // runner
	if DebugLevel() >= 1 {
...
func (c *server2Conn) recvInFrame() (*http2InFrame, error) {
	...
	inFrame.effecFrom = c.partBack
	inFrame.effecEdge = c.partFore
...
func (c *server2Conn) _growInFrame(size uint32) error {
...
func (c *server2Conn) _joinContinuations(headers *http2InFrame) error { // into a single headers frame
	headers.inBuffer = nil // will be restored at the end of continuations
	...
	for { // each continuation frame
		...
		if err := continuation.decodeHeader(c.inBuffer.buf[c.contBack:c.contFore]); err != nil {
			return err
		}
		...
		copy(c.inBuffer.buf[headers.effecEdge:], c.inBuffer.buf[c.contBack:c.contFore]) // may overwrite padding if exists
		headers.effecEdge += continuation.length
		headers.length += continuation.length // we don't care if padding is overwritten. just accumulate
		c.partFore += continuation.length     // also accumulate headers payload, with padding included
		// End of headers?
		if continuation.endHeaders {
			headers.endHeaders = true
			headers.inBuffer = c.inBuffer // restore the inBuffer
...
func (c *server2Conn) _growContinuation(size uint32, headers *http2InFrame) error {
	...
	if c.contFore > c.inBuffer.size() { // needs slide
		...
		headers.effecFrom -= c.partBack
		headers.effecEdge -= c.partBack
		c.partBack = 0
...
func (c *server2Conn) _fillInBuffer(size uint32) error {
...
func (c *server2Conn) goawayCloseConn(h2e http2Error) {
	...
	binary.BigEndian.PutUint32(goaway.outBuffer[0:4], c.lastStreamID)
	binary.BigEndian.PutUint32(goaway.outBuffer[4:8], uint32(h2e))
	goaway.payload = goaway.outBuffer[0:8]
	c.sendOutFrame(goaway) // ignore error
	goaway.zero()
	c.closeConn()
}

var server2InFrameProcessors = [...]func(*server2Conn, *http2InFrame) error{
...
func (c *server2Conn) _updateClientSettings(settingsInFrame *http2InFrame) error {
...
func (c *server2Conn) processPingInFrame(pingInFrame *http2InFrame) error {
	...
	pong.ack = true
	pong.payload = pingInFrame.effective()
	err := c.sendOutFrame(pong)
	pong.zero()
	return err
}
...
func (c *server2Conn) sendOutFrame(outFrame *http2OutFrame) error {
...
type backend2Conn struct {
	...
	rawConn        syscall.RawConn     // for syscall. only usable when netConn is TCP/UDS
	inBuffer       *http2InBuffer      // http2InBuffer in use, for receiving incoming frames
	serverSettings http2Settings       // settings of remote server
	table          http2DynamicTable   // dynamic table
	incomingChan   chan any            // ...
	inWindow       int32               // ...
	outWindow      int32               // ...
	outgoingChan   chan *http2OutFrame // ...
	// Conn states (zeros)
...
type _backend2Conn0 struct { // for fast reset, entirely
	inBufferEdge uint32                            // ...
	streamIDs    [http2MaxActiveStreams + 1]uint32 // ...
	nInFrames    int64                             // num of incoming frames
	nStreams     int8                              // num of active streams
}
...
func (c *backend2Conn) MakeTempName(to []byte, unixTime int64) int {
...
type http2InFrame struct { // 32 bytes
	...
	effecFrom  uint32         // (effective) payload from
	effecEdge  uint32         // (effective) payload edge
}
...
func (f *http2InFrame) decodeHeader(header []byte) error {
	...
	if f.streamID > 0 && f.streamID&0x1 == 0 { // we don't support server push, so only odd stream ids are allowed
		return http2ErrorProtocol
	}
	f.kind = header[3]
	flags := header[4]
...
func (f *http2InFrame) effective() []byte { return f.inBuffer.buf[f.effecFrom:f.effecEdge] } // effective payload
...
func (f *http2InFrame) checkAsData() error {
	...
	if f.padded {
		padLength = uint32(f.inBuffer.buf[f.effecFrom])
		othersLen += 1
		f.effecFrom += 1
	}
