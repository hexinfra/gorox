...
type http2Conn_[S http2Stream] struct { // for server2Conn and backend2Conn
	...
	freeSeats    [http2MaxConcurrentStreams]uint8 // a stack holding free seats in c.activeStreams and c.activeStreamIDs
	freeSeatsTop uint8                            // stack top of c.freeSeats
	// Conn states (zeros)
...
func (c *http2Conn_[S]) appendStream(stream S) {
	seat := c._getFreeSeat()
	stream.setSeat(seat)
...
func (c *http2Conn_[S]) searchStream(streamID uint32) S {
...
func (c *http2Conn_[S]) retireStream(stream S) {
	seat := stream.getSeat()
	if DebugLevel() >= 2 {
