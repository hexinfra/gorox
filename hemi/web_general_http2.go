...
type http2Conn_[S http2Stream] struct { // for server2Conn and backend2Conn
	...
	decodeTable  http2DynamicTable                // <= 5 KiB. hpack dynamic table used for decoding field blocks received from remote
	...
	encodeTable  http2DynamicTable                // <= 5 KiB. hpack dynamic table used for encoding field list sent to remote
	// Conn states (zeros)
...
func (c *http2Conn_[S]) hpackDecode(fields []byte, join func(p []byte) bool) bool { // TODO: method value escapes to heap?
	...
	var (
		I  uint32 // the integer
		j  int    // index of fields
		ok bool   // decode succeeds?
	)
	i, n := 0, len(fields)
	for i < n {
		b := fields[i]
		if b >= 0b_1000_0000 { // 6.1. Indexed Header Field Representation
		...
		} else if b >= 0b_0100_0000 { // 6.2.1. Literal Header Field with Incremental Indexing
			...
			fieldValue, j, ok = http2DecodeString(fields[i:])
			if !ok {
				Println("decode error")
				return false
			}
		...
		} else if b >= 0b_0010_0000 { // 6.3. Dynamic Table Size Update
		...
		} else if b >= 0b_0001_0000 { // 6.2.3. Literal Header Field Never Indexed
