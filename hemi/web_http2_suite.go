...
func (c *server2Conn) _joinContinuations(headers *http2InFrame) error { // into a single headers frame
	...
	for { // each continuation frame
		...
		copy(c.inBuffer.buf[headers.realEdge:], c.inBuffer.buf[c.contBack:c.contFore]) // may overwrite padding if exists
		headers.realEdge += continuation.length
...
func (c *server2Conn) _growContinuation(size uint32, headers *http2InFrame) error {
	...
	if c.contFore > c.inBuffer.size() { // needs slide
		...
		headers.realFrom -= c.partBack
		headers.realEdge -= c.partBack
		c.partBack = 0
...
func (c *http2Conn_) onGet(id int64) {
	c.webConn_.onGet(id)
...
func (s *http2Stream_) onUse(id uint32) {
	s.webStream_.onUse()

	s.id = id
}
func (s *http2Stream_) onEnd() {
	s._http2Stream0 = _http2Stream0{}

	s.webStream_.onEnd()
}

func (s *http2Stream_) getID() uint32 { return s.id }

func (s *http2Stream_) getIndex() uint8      { return s.index }
func (s *http2Stream_) setIndex(index uint8) { s.index = index }
