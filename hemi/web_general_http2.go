...
type http2Conn_[S http2Stream] struct { // for server2Conn and backend2Conn
	...
	outFrame http2OutFrame[S] // used by c.manage() to send special out frames. immediately reset after use
	...
	outgoingChan  chan *http2OutFrame[S]           // frames generated by streams and waiting for c.manage() to send
	freePositions [http2MaxConcurrentStreams]uint8 // a stack holding free positions in c.activeStreamIDs
	freeStackTop  uint8                            // stack top of c.freePositions
	// Conn states (zeros)
	activeStreams [http2MaxConcurrentStreams]S // active (open, remoteClosed, localClosed) streams
...
func (c *http2Conn_[S]) onGet(id int64, holder holder, netConn net.Conn, rawConn syscall.RawConn) {
	...
	c.freePositions = http2FreePositions
	c.freeStackTop = 255 // +1 becomes 0
}
...
func (c *http2Conn_[S]) receive() { // runner
	if DebugLevel() >= 1 {
...
func (c *http2Conn_[S]) recvInFrame() (*http2InFrame, error) { // excluding pushPromise, which is not supported, and continuation, which cannot arrive alone
	...
	if c.cumulativeInFrames == 20 && !c.acknowledged { // TODO: change this policy?
...
func (c *http2Conn_[S]) onContinuationInFrame(continuationInFrame *http2InFrame) error {
	panic("discrete continuation frames should be rejected priorly")
}
...
func (c *http2Conn_[S]) joinStream(stream S) {
...
func (c *http2Conn_[S]) findStream(streamID uint32) S {
...
func (c *http2Conn_[S]) quitStream(stream S) {
	position := stream.getPosition()
	if DebugLevel() >= 2 {
		Printf("conn=%d quitStream=%d at %d\n", c.id, stream.nativeID(), position)
	}
	var s S // nil
	c.activeStreams[position] = s
...
func (c *http2Conn_[S]) _decodeFields(fields []byte, join func(p []byte) bool) bool { // TODO: method value escapes to heap?
	var (
		I  uint32
		j  int
		ok bool
...
func (c *http2Conn_[S]) _findStreamID(streamID uint32) uint8 {
	c.activeStreamIDs[http2MaxConcurrentStreams] = streamID // the stream id to search for
	position := uint8(0)
	for c.activeStreamIDs[position] != streamID {
		position++
	}
	return position
}
func (c *http2Conn_[S]) _getFreePosition() uint8 {
	c.freeStackTop++
	return c.freePositions[c.freeStackTop]
}
func (c *http2Conn_[S]) _putFreePosition(position uint8) {
	c.freePositions[c.freeStackTop] = position
	c.freeStackTop--
}
...
type http2Stream interface {
	...
	getPosition() uint8         // at activeStreamIDs
	setPosition(position uint8) // at activeStreamIDs
}
...
type _http2Stream0 struct { // for fast reset, entirely
	position uint8 // the position at s.conn.activeStreamIDs
...
func (s *http2Stream_[C]) getPosition() uint8         { return s.position }
func (s *http2Stream_[C]) setPosition(position uint8) { s.position = position }
